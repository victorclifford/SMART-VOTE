...
const origin = ["https://smartvote-app.vercel.app", "http://localhost:3000"];
// Middlewares
  ...
  try {
    ...
    io.on("connection", (socket) => {
      ...
      socket.on("voted", async (data) => {
        ...
        if (!election || !voter || !candidate)
          return socket
            .to(socket.id)
            .emit("error-vote", "Include an election, voter or candidate");

        const ele = await Election.findById(election);

        if (ele.voters.includes(voter))
          return socket.emit("error-vote", "You have voted before");
        try {
          ...
          await Election.findByIdAndUpdate(election, {
